<template>
  <div>
    <hyper-table ref="table" :columns="options.columns" :filters="options.filters" url="/channels"
      :ui="{ th: { base: 'text-left' } }">
      <template #id-data="{ row }">
          <NuxtLink class="text-primary dark:text-gray-400 font-bold underline"
            :to="{ name: 'channels-id', params: { id: row.id } }">
            {{ row.id }}
          </NuxtLink>
      </template>
      ...
      <template #merchant-data="{ row }">
        <div class="grid grid-cols-12 gap-1">
          ...
          <UBadge v-if="!row.merchant.reviewed" class="w-10 col-span-2 rounded-full h-5" size="xs" label="New" color="red" />
          <NuxtLink class="text-primary dark:text-gray-400 col-span-10 font-bold underline"
      ...
      <template #extra-header>
        <UButton size="xs" @click="openSide = true" v-if="$can('create channels')" label="Add Merchant" />
      </template>
    </hyper-table>
...
export default {
  ...
  methods: {
    createResource() {
      useApiFetch(`/channels`, { method: "POST", body: this.resource }).then(
        () => {
          this.openSide = false;
