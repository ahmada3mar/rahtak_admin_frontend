<template>
    <div v-if="env == 'development'" class="h-screen w-screen flex flex-col gap-28 p-20">
        <h5>{{ error.message }}</h5>
        <div v-html="error.stack"></div>
    </div>
    <div v-else class="flex h-[calc(100vh-80px)] items-center justify-center p-5 w-full bg-white-bk">
        <div class="text-center">
            ...
            <p class="text-slate-600 mt-5 lg:text-lg">Oops something went wrong. Try to refresh this page or <br> feel free
                to contact us if the problem presists.</p>
        </div>
    </div>
</template>
